...
- **The catalogue is data.** [registry.js](public/js/registry.js) holds every category and its exhibits' metadata and imports nothing; array order is display order, and an entry marked `soon` has no directory and nothing routes to it. A category's `blurb` describes the *form* — it sits in the rail above every design of it and outlives any one of them — while an exhibit's describes what that design does to you. An `id` is a short slug for the URL and is not derived from the label; a label that doesn't shorten into one is free to take a different word.
...
A category's fields live in one `form.js` beside its exhibits, and each re-exports them as its own `fields`. What else that file holds is the category's own business: payment's exhibits both walk the form position by position, so it also carries the length of the whole thing and the slicing they share, and a category whose exhibits need neither says only `fields`.
...
- Serve with `python3 -m http.server -d public` from the repo root. Do not start a server on the user's behalf without being asked.
  - The modules are served as they are, so a browser can run a **half-stale module graph**: one cached file beside a fresh one links without error as long as the names it imports still exist, and then misbehaves silently and convincingly. A bug the suite cannot reproduce is this until proven otherwise. The tell is the server handing out the change while the running page misses it, and a reload need not settle it — the reliable read is a fresh origin, `127.0.0.1` where the server was `localhost` or the other way round, since neither carries the other's HTTP cache.
- `rsvg-convert -w 1200 -h 630 og-image.svg -o public/og-image.png && oxipng -o 2 --strip safe public/og-image.png` — the social card. [og-image.svg](og-image.svg) is the source and stays out of `public/`, since a scraper wants the raster; edit the SVG and re-run rather than touching the PNG. It draws the site's own reel row, so a change to the cell's colours or proportions is a change here too.
- `rsvg-convert -w 48 -h 48 public/favicon.svg -o public/favicon-48.png`, and the same at 180 into `public/apple-touch-icon.png`, each with the same `oxipng` pass after it — the raster icons. Here the vector is served as well as rastered, so unlike the card it belongs in `public/`; edit it and re-run rather than touching the PNGs. It is drawn on a 32 grid on whole units, which is what keeps the columns from blurring into each other at 16px.
- A raster is committed compressed or not at all: git keeps every version forever, so a later optimisation pass leaves both copies in history. `-o 2 --strip safe` is lossless, and like Prettier's the version decides the bytes, so a rebuild on a different oxipng is a different file rather than a corrected one.
...
- **`.actions`, `.tally` and `.hint`** are a row carrying whatever commits a value, the count of what is left, and the note underneath. An exhibit that commits the form in one place ends with the three of them; one that commits a field at a time takes an `.actions` and a `.tally` per field and keeps the `.hint` for the bottom. An exhibit reaches for them rather than restating them, and adds a class of its own only where it has something to add — the slot machine rules its actions row off from the board above it, and says only that. `.tally` sets tabular figures, since a count that changes under the visitor shouldn't shift while it does. What is deliberately *not* shared is each exhibit's digit-group wrapper: two declarations coinciding is not agreement, and the shell owning a class only exhibits use would buy indirection instead.
