...
export const fields = [
  { name: "number", label: "Card number", length: DIGITS, group: 4, benchmarkMs: 9000 },
];
...
export function mount(root, api) {
  ...
  const wheels = Array.from({ length: DIGITS }, () =>
    el(
      "span",
      { class: "odo-strip" },
  ...
  const cells = wheels.map((wheel) => el("span", { class: "odo-cell" }, wheel));
  const readout = el("div", { class: "odo-readout", role: "img" }, ...grouped(cells));
  ...
  const digitInput = el("input", {
    type: "range",
    class: "odo-range odo-range-digit",
    id: "odo-digit",
    min: 0,
    max: MAX_DIGIT,
    step: 1,
  ...
  function commit() {
    ...
    api.set("number", report(state));
    touch();
    render();
    api.announce(`${place} set to ${digit}. ${remaining(state)} of ${DIGITS} still to enter.`);
  }
  ...
  function render() {
    ...
    readout.setAttribute("aria-label", `Card number so far: ${spellOut(state)}`);

    placeInput.value = String(state.place);
    placeInput.setAttribute(
      "aria-valuetext",
      `${PLACES[state.place]}, digit ${state.place + 1} of ${DIGITS}, ${settledPhrase()}`,
    );
    ...
    tally.textContent = `${remaining(state)} of ${DIGITS} still to enter`;
  }
...
function grouped(cells) {
  const out = [];
  for (let at = 0; at < cells.length; at += GROUP) {
    out.push(el("span", { class: "odo-group" }, ...cells.slice(at, at + GROUP)));
  }
  return out;
}
